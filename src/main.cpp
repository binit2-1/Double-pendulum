...
void DrawDoublePendulum(float l1, float l2, Vector2 start_pos, float theta1, float theta2)
{
    ...
    traceQueue.push_back(end_pos2);
    if (traceQueue.size() > TRACE_LENGTH) {
        traceQueue.pop_front(); // Remove the oldest point
    }

    for (const auto& point : traceQueue) {
